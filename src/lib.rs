...
//! - **Browser**：调用本地 headless 浏览器执行 JS 后取 HTML（默认探测 Chrome / Chromium）
...
//! 没有可用的浏览器实例（`autodetect` 没找到 Chrome、也没手动注入），
...
pub use browser::{BrowserFetch, ChromeBrowser, LightpandaBrowser, VIRTUAL_TIME_BUDGET_DEFAULT, autodetect_browser};
